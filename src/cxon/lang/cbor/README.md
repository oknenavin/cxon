...
macros for binding of enumeration and class types:
  ...
  - [`class types`][cpp-class]
    ...
    CXON_CBOR_CLS_READ(my_struct,
        CXON_CBOR_CLS_FIELD_ASIS(first),
        CXON_CBOR_CLS_FIELD_NAME("second field", second),
        CXON_CBOR_CLS_FIELD_SKIP("skip")
    )
    CXON_CBOR_CLS_WRITE(my_struct,
        CXON_CBOR_CLS_FIELD_ASIS(first),
        CXON_CBOR_CLS_FIELD_NAME("second field", second)
    )
    ...
    my_struct v0 = { 1, 2, 3 };
    std::basic_string<unsigned char> s0;
        cxon::to_bytes(s0, v0);
    assert(s0 == "TODO");
    my_struct v1;
        cxon::from_bytes(v1, "TODO");
    assert(v1 == v0);
    ```
