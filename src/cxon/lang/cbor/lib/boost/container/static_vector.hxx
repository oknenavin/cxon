...
#ifndef CXON_CBOR_LIB_BOOST_CONTAINER_STATIC_VECTOR_HXX_
#define CXON_CBOR_LIB_BOOST_CONTAINER_STATIC_VECTOR_HXX_
...
namespace cxon {
    ...
    template <typename X, typename T, std::size_t N>
        struct read<CBOR<X>, boost::container::static_vector<T, N>> {
    ...
    template <typename X, typename T>
        struct read<CBOR<X>, boost::container::static_vector<T, 0>> {
            template <typename II, typename Cx, typename Y = CBOR<X>>
                static bool value(boost::container::static_vector<T, 0>& t, II& i, II e, Cx& cx) {
                    return cbor::cnt::read_array<Y>(t.begin(), t.end(), i, e, cx);
                }
        };

    template <typename X, typename T, std::size_t N>
        struct write<CBOR<X>, boost::container::static_vector<T, N>> {
